...
Chain::Chain(pugi::xml_node node) {
    ...
    for (pugi::xml_node tool : node.children("nuclide")) {
        ...
        if (it != nuclides.end()) {
            ...
            nuclides.push_back(std::unique_ptr<ChainNuclide>(
                new ChainNuclide(tool)));  //! TODO make_unique
            num = nuclides.size() - 1;
    ...
    for (auto& item : this->name_idx) {
        for (auto& elem : nuclides[item.second]->get_decaybr()) {
            out.push_back(std::make_tuple(
                item.second, this->name_idx[elem.first], elem.second));
        }
    }
    return out;
}
...
*
  ...
  {
      std::cout << react_map["(n,gamma)"][i].first << "  " <<
react_map["(n,gamma)"][i].second << "  "<< 1 << "  " <<std::endl;
  }
  output:
0  1  1
1  2  1
...
std::map<std::string, std::vector<std::pair<int, int>>> Chain::form_reaction() {
    ...
    for (auto& item : this->name_idx) {
        for (auto& elem : nuclides[item.second]->get_reactions()) {
            auto search =
                std::find(reactionlist.begin(), reactionlist.end(), elem.first);
            auto searchelem =
                std::find_if(name_idx.begin(), name_idx.end(),
                             [elem](std::pair<std::string, size_t> index) {
            ...
            if (searchelem != name_idx.end()) {
                if (search == reactionlist.end()) {
                    ...
                    out[elem.first].push_back(std::make_pair(
                        item.second, this->name_idx[elem.second]));
                }
            }
        }
    }
...
* To test this parser (only Pu239 have 2000000.0):
  ...
  {
      std::cout << map[2000000.0][i][0] << "  " << map[2000000.0][i][1] << " "<<
map[2000000.0][i][2] << "  " <<std::endl;
  }
output
...
3557  1346  0
...
Chain::form_yield_map() {
    std::map<double, std::vector<std::tuple<int, int, double>>> out;
    ...
    for (int i = 0; i < nuclides.size(); i++) {
        for (auto& item : nuclides[i]->get_nfy_energies()) {
            if (out.count(item) == 0) out.insert({item, insertion});
        }
    }
    for (int i = 0; i < nuclides.size(); i++) {
...
std::pair<std::vector<double>, std::vector<double>> Chain::get_yield_map(
    size_t father, const std::string& daughter) {
    ...
    for (int j = 0; j < nuclidenergies.size(); j++) {
        ...
        if (it != n_map.end()) {
            fracenergy_.push_back(
                std::make_pair(nuclidenergies[j], it->second));
        }
    }
...
void read_chain_xml(const std::string& filename) {
    ...
    chain = std::make_unique<Chain>(chain_node);
}
...
extern "C" int openbps_set_nuclide_name_idx(const char* name, size_t idx) {
    int err = 0;
    try {
...
extern "C" int openbps_get_nuclide_index(const char* name, size_t* idx) {
    int err = 0;
    try {
...
extern "C" int openbps_delete_nuclide_by_name(const char* name) {
    int err = 0;
    try {
